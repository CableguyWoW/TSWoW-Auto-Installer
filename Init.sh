...
echo "##########################################################"
echo ""
sudo find /TSWoW-Auto-Installer/ -type d -name ".git" -prune -o -type f -exec dos2unix {} \;
sudo chmod -R 777 /TSWoW-Auto-Installer/
cd /TSWoW-Auto-Installer/scripts/Setup/
fi
...
if [ "$RANDOMIZE_PASSWORDS" = "true" ]; then
    replace_randomizepass "/TSWoW-Auto-Installer/configs/*"  # Example: replace in all .txt files
else
    ...
    if [ "$REMOTE_DB_SETUP" = "true" ]; then
        ...
        while true; do
            ...
            if [[ "$yn" =~ ^[Yy]$ ]]; then
                ...
                CONFIG_FILE="/TSWoW-Auto-Installer/configs/root-config"  # Define the config file path
...
echo "##########################################################"
echo ""
echo "All passwords are stored in - /TSWoW-Auto-Installer/configs/"
if [ "$RANDOMIZE_PASSWORDS" = "true" ]; then
...
echo ""
echo "cd /TSWoW-Auto-Installer/scripts/Setup/ && ./Root-Install.sh all"
echo ""
echo "##########################################################"
echo ""
fi
